...
const otokayit = require("../SystemModels/otokayit");
...
module.exports = async (member) => {
  ...
  if (tagges.some(tag => member.user.tag.includes(tag))) { member.setNickname(`${ayar.ikinciTag} İsim ' Yaş`).catch(); }
 ...
 const tagModedata = await regstats.findOne({ guildID: allah.GuildID })
  if (tagModedata && tagModedata.tagMode === false) {
    if (otoreg) {
      ...
      await member.setNickname(`${ayar.ikinciTag} ${otoreg.name} ' ${otoreg.age}`);
  ...
  if(res.taglar.some(x => member.user.tag.includes(x))) { 
    ...
    if (allah.dmMessages) member.send({ content:`${member.guild.name} adlı sunucumuza olan erişiminiz engellendi! Sunucumuzda yasaklı olan bir simgeyi (${x}) isminizde taşımanızdan dolayıdır. Sunucuya erişim sağlamak için simgeyi (${x}) isminizden çıkartmanız gerekmektedir.\n\nSimgeyi (${x}) isminizden kaldırmanıza rağmen üstünüzde halen Yasaklı Tag rolü varsa sunucudan gir çık yapabilirsiniz veya sağ tarafta bulunan yetkililer ile iletişim kurabilirsiniz. **-Yönetim**\n\n__Sunucu Tagımız__\n**${conf.tag}**`}).catch(() => {});
}
}) 
if (!usedInvite) {
  ...
  Hesabın __${memberGün} ${memberAylar} ${memberTarih}__ tarihinde oluşturulmuş: ${guvenilirlik ? `Şüpheli! ${red}` : `Güvenli! ${green}` }
  ...
  \`\`\`
  ${tagModedata ? tagModedata.tagMode === true ? `(**Şuan da taglı alımdayız**)`:``:``} 
  `});
channel.wsend({ content:`${member} üyesi sunucumuza **Özel URL** kullanarak <t:${Number(String(Date.now()).substring(0, 10))}:R> katıldı!`})
return }
...
channel.send({ content:`${member} üyesi ${usedInvite.inviter.tag} tarafından <t:${Number(String(Date.now()).substring(0, 10))}:R> sunucumuza davet edildi. **(**Toplam Davet: \`${total}\`**)**`})
